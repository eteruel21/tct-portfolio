    ...
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-[#1A1A1A]">¿Qué deseas?</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {data.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 py-6" aria-label="Galería de proyectos">
            {data.map((it) => (
              <li key={it.id} className="rounded-2xl overflow-hidden border border-gray-200 shadow-md bg-white hover:shadow-lg transition-all">
                <figure className="flex flex-col h-full">
                  ...
                  <figcaption className="p-4 flex flex-col justify-between flex-grow">
                    <div>
                      <Link to={`/proyecto/${it.id}`} className="font-semibold text-[#1A1A1A] text-base md:text-lg hover:underline">
                        {it.titulo}
                      </Link>
                    ...
                    <div className="mt-3">
                      <Link
                        to={`/cotizador?mensaje=${encodeURIComponent(`Hola, estoy interesado en el proyecto: ${it.titulo}`)}`}
